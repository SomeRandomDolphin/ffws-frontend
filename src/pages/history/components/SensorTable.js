    ...
    return (
      <div className="my-8">
        <div className="rounded-lg min-w-[1000px] border grid grid-cols-12 overflow-hidden text-sm bg-white shadow">
          ...
          <div className="col-span-3 flex items-center px-5 text-left  border-blue-500 bg-blue-100 py-2 font-semibold">
          ...
          {sensorData.map((item, i) => (
            <div className="col-span-12 grid grid-cols-12" key={i}>
              ...
              <div className="col-span-3 px-5 text-left border-t py-2">
                {stasiun === "Cendono"
                  ? item.curah_hujan_cendono < 0.6
                    ? "Hujan Ringan"
                    : item.curah_hujan_cendono < 2.0
                      ? "Hujan Sedang"
                      : "Hujan Lebat"
                  : item.curah_hujan_lawang < 0.6
                    ? "Hujan Ringan"
                    : item.curah_hujan_lawang < 2.0
                      ? "Hujan Sedang"
                      : "Hujan Lebat"}
              </div>
              <div className="col-span-2 px-5 text-left border-t py-2">
                {stasiun === "Cendono"
                  ? item.curah_hujan_cendono
                  : item.curah_hujan_lawang}
              </div>
            </div>
          ))}
          ...
          <div className="col-span-4 border-t p-2">
            <div className="flex items-center">
              ...
              <p className="text-sm">
                Displaying{" "}
                {`${pageIndex * 10 + 1} - ${sensorData.length < (pageIndex + 1) * 10 ? pageIndex * 10 + (sensorData.length % 10) : (pageIndex + 1) * 10} `}{" "}
                out of {totalLength}
              </p>
            </div>
          </div>
